...
void module::finalize_header(std::ostream &out, std::shared_ptr<const module_instance> context) {
...
top::top(std::string_view name)
    : root(
          ...
          [identifier_generator = _identifier_generator, var_map = _var_map](
              const std::string_view full_path, scope_fn::dumper_fn fn) -> value_context {
            ...
            // Create a function that allows the registration in this class to be reset by the
            // variable destructor.
...
void top::log_time(std::ostream &out, scope_fn::sequence_t new_time, bool force,
...
void top::finalize_header(std::ostream &out,
...
void top::time_update_core(std::ostream &out) {
  ...
  for (auto [identifier, dump_fn] : _var_map->dumper_map) {
    ...
    if (sequence.next.has_value()) {
      ...
      if constexpr (SIMPLE_VCD_DEBUG) {
        out << "$comment first pass found: " << identifier << " @ " << sequence.next.value()
            << " $end\n";
      }
    }
  ...
  while (status.size() > 0) {
    ...
    for (const auto &identifier : identifiers) {
      ...
      if (done_sequence.next.has_value()) {
        ...
        if constexpr (SIMPLE_VCD_DEBUG) {
          out << "$comment second pass found: " << identifier << " @ " << sequence << " -> "
              << done_sequence.next.value() << " $end\n";
        }
      } else {
        if constexpr (SIMPLE_VCD_DEBUG) {
          out << "$comment second pass not found: " << identifier << " @ " << sequence << " $end\n";
        }
      }
    }
...
template <typename T>
void value_base::dump(std::ostream &out, const size_t bit_size, const value_state state,
                      const T value) const {
  if constexpr (SIMPLE_VCD_DEBUG) {
...
template void value_base::dump<bool>(std::ostream &out, const size_t bit_size,
                                     const value_state state, const bool value) const;

template void value_base::dump<int>(std::ostream &out, const size_t bit_size,
                                    const value_state state, const int value) const;
template void value_base::dump<unsigned int>(std::ostream &out, const size_t bit_size,
                                             const value_state state,
                                             const unsigned int value) const;

template void value_base::dump<char>(std::ostream &out, const size_t bit_size,
                                     const value_state state, const char value) const;
template void value_base::dump<unsigned char>(std::ostream &out, const size_t bit_size,
                                              const value_state state,
                                              const unsigned char value) const;

template void value_base::dump<short>(std::ostream &out, const size_t bit_size,
                                      const value_state state, const short value) const;
template void value_base::dump<unsigned short>(std::ostream &out, const size_t bit_size,
                                               const value_state state,
                                               const unsigned short value) const;

template void value_base::dump<long>(std::ostream &out, const size_t bit_size,
                                     const value_state state, const long value) const;
template void value_base::dump<unsigned long>(std::ostream &out, const size_t bit_size,
                                              const value_state state,
                                              const unsigned long value) const;

template void value_base::dump<float>(std::ostream &out, const size_t bit_size,
                                      const value_state state, const float value) const;
template void value_base::dump<double>(std::ostream &out, const size_t bit_size,
                                       const value_state state, const double value) const;
