...
/**
   ...
   As VCD is format used for hardware design there is a number phases used to build a representation
...
/**  This class represents te interface between classes in the VCD tracer module.
     As the VCD tracer is implemented using the dependency injection style these functions and types
   are used instead of a class hierachy.
*/
class scope_fn {
 public:
  ...
  //! The type signature of a function used to add a variable to be traced. The parameter define the
  //! information reqired VCD by a VCD header.
  using add_fn = std::function<struct value_context(
      std::string_view var_name, std::string_view var_type, unsigned int bit_size, dumper_fn fn)>;
  //! The type signature of a function used to register a variable and it's dumper.
  using register_fn = std::function<struct value_context(std::string_view var_name, dumper_fn fn)>;
...
/** Represent a module within a hierarchical scope.
    ...
    This class only captures the child modules, on the assumption we will traverse from the top
   module. It is expected this class is only used during the elaboration phase, and can be freed
   once all variables are assigned identifers.
 */
class module_instance {
 public:
   ...
   */
  module_instance(const std::string_view name) : instance_name(name) {}
  //! The name of this module instance.
  const std::string instance_name;
  //! This string buffer will capture the VCD header information, it is written to file at the end
  //! of elaboration.
...
/** Specialized type trait to indicate boolean values are traced as one bit regardless of the C++
 * storage size.
 */
template <>
...
class value_base {
  ...
  */
  virtual void elaborate(scope_fn::add_fn add_fn, const std::string_view var_name) = 0;

 protected:
  /** Create a new value to be traced, defining it at compile time.
      A value cannot be created without type information, so this must be called by a concrete child
     class constructora.
  ...
  */
  value_base(const unsigned int bit_size, const char *var_type, scope_fn::add_fn add_fn,
             const std::string_view var_name, scope_fn::dumper_fn dumper_fn)
  ...
  */
  ...
  /**
      ...
      @param dumper_fn The specialized function that can be used to dump this value to file.
   */
  void elaborate_base(const unsigned int bit_size, const char *var_type, scope_fn::add_fn add_fn,
                      const std::string_view var_name, scope_fn::dumper_fn dumper_fn) {
...
/** Typed value for tracing representation.
    ...
    @tparam TRACE_DEPTH - When set to more than one a buffer of values can be accumulated before
   writing to file.
    ...
    The lifetime of this class is the full trace lifetime.
 */
template <typename T, unsigned int BIT_SIZE = bit_size<T>::value, int TRACE_DEPTH = 1,
...
class value : public value_base {
 ...
 public:
   ...
   */
  value(void) : value_base(BIT_SIZE) {
    _idx.write = -1;
  ...
  /** Instanciate a trace value with an initialized value. The state will be set to known.
      ...
      The name and scope need to be set later via elaborate().
   */
  value(const T default_value) : value_base(BIT_SIZE) {
    _idx.write = -1;
    _samples[0].set(default_value);
  }
  /** Instanciate named and scoped trace value with an uninitialized value. The state will be set to
     unknown
      @param var_name variable name
      @param add_fn Funtion to register this trace variable with it's scope.
  */
  value(scope_fn::add_fn add_fn, const std::string_view var_name)
      : value_base(BIT_SIZE, vcd_var_type<T>::value, add_fn, var_name,
                   [this](std::ostream &out, bool start) -> scope_fn::dump_sequence_t {
                     return this->dump(out, start);
                   }) {
    _idx.write = -1;
  }
  /** Instanciate named and scoped trace value with an initialized value. The state will be set to
     known.
      @param var_name variable name
      @param add_fn Funtion to register this trace variable with it's scope.
      @param default_value Initial value to be traced at time 0.
  */
  value(scope_fn::add_fn add_fn, const std::string_view var_name, const T default_value)
      : value_base(BIT_SIZE, vcd_var_type<T>::value, add_fn, var_name,
                   [this](std::ostream &out, bool start) -> scope_fn::dump_sequence_t {
                     return this->dump(out, start);
                   }) {
  ...
  /** Elaborate a value by settings it's name and scope
      ...
      @param default_value Initial value to be traced at time 0.
  */
  virtual void elaborate(scope_fn::add_fn add_fn, const std::string_view var_name) override {
    elaborate_base(BIT_SIZE, vcd_var_type<T>::value, add_fn, var_name,
                   std::bind(&value<T, BIT_SIZE, TRACE_DEPTH, CUR_SEQ>::dump, this,
  ...
  void set_state(void) {
    if constexpr (TRACE_DEPTH == 1) {
      ...
      if ((_idx.write == -1) ||
          ((_idx.write < TRACE_DEPTH) &&
           (_samples[static_cast<size_t>(_idx.write % TRACE_DEPTH)].state != S))) {
        // The trace needs updating
        if ((_idx.write == -1) ||
            (_samples[static_cast<size_t>(_idx.write % TRACE_DEPTH)].sequence != *CUR_SEQ)) {
  ...
  */
  void set(const T v) {
    if constexpr (TRACE_DEPTH == 1) {
      ...
      if ((_idx.write == -1) ||
          ((_idx.write < TRACE_DEPTH) &&
           ((sample_changed(v, _samples[static_cast<size_t>(_idx.write % TRACE_DEPTH)].value)) ||
            _samples[static_cast<size_t>(_idx.write % TRACE_DEPTH)].state != value_state::known))) {
        if ((_idx.write == -1) ||
            (_samples[static_cast<size_t>(_idx.write % TRACE_DEPTH)].sequence != *CUR_SEQ)) {
...
class module {
  ...
  */
  ...
  /** Declare a module instance, and provide the parent scope a reference to another module.
      ...
      @param instance_name The name of this module instance.
  */
  module(module &parent, std::string_view instance_name) :_register_fn{parent.get_register_fn()},
      _context{std::make_shared<module_instance>(instance_name)} {
    _context->vcd_scope << "$scope module " << instance_name << " $end\n";
 ...
 private:
  /** Declare a module instance from within a different module instance,
      ...
      @param instance_name The name of this module instance.
  */
  module(scope_fn::register_fn register_fn, std::string_view instance_name,
 ...
 public:
  /** Elaborate a trace variable within this module. The varible should have been instanciated
     WITHOUT scope.
  ...
  */
  ...
  /** Get the function to be provided to a trace variable declaration to give it scoped withing this
   * module instance
   */
  [[nodiscard]] scope_fn::add_fn get_add_fn(void) {
    return [this](std::string_view var_name, std::string_view var_type, const unsigned int bit_size,
 ...
 private:
  // This function passed a new variable up in the heirarchy to be assinged a global identifier and
  // registered.
 ...
 private:
  void finalize_header(std::ostream &out, std::shared_ptr<const module_instance> context);

  /** A function that can be passed to a new trace variable to declare it within the scope of this
   * module instance
   */
  [[nodiscard]] value_context add_var(std::string_view var_name, std::string_view var_type,
                                      const unsigned int bit_size, scope_fn::dumper_fn fn) {
    std::string child_path = _context->instance_name + "." + std::string(var_name);
    auto value_context = _register_fn(child_path, fn);
    _context->vcd_scope << "$var " << var_type << " " << bit_size << " " << value_context.identifier
                        << " " << var_name << " $end\n";
  ...
  [[nodiscard]] scope_fn::register_fn get_register_fn(void) {
    return [this](std::string_view child_path, scope_fn::dumper_fn fn) -> value_context {
      return this->register_var(child_path, fn);
    };
  }
  /** Get the function that can be used to register a variable within this module
      @param child_path The path of the variable. It will be added to the full instance path to be
     registered at the next level.
      @param fn         The function that will be used to dump a given variable to file.
  */
  [[nodiscard]] value_context register_var(std::string_view child_path, scope_fn::dumper_fn fn) {
...
class top {
  ...
  */
  void finalize_header(std::ostream &out, std::chrono::time_point<std::chrono::system_clock> date);
 ...
 private:
  ...
  void log_time(std::ostream &out, scope_fn::sequence_t new_time, bool force,
                std::string_view reason);
  // Creation of identifiers
  std::shared_ptr<identifier_generator> _identifier_generator =
      std::make_shared<identifier_generator>();
