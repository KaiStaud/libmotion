...
class TrapezoidalRamp {
 public:
  TrapezoidalRamp(motion_profile::TargetConstraints constraints)
    ...
    const motion_profile::TargetConstraints constraints_up{.acceleration = constraints.acceleration,
  ...
  auto GetSegment(uint32_t t) -> Segment {
    if (t <= t_accelerate_) {
      ...
      return Segment::kConstant;
               ...
               ((t <= (t_accelerate_ + t_decellerate_ + t_const_)))) {
